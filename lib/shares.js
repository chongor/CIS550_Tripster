...
module.exports = function(db){
	...
	this.getMultimedia = function(sid, callback){
		db.query('SELECT M.id as id, M.title as title, M.url as url, S.time as time, S.type as type, U.name as owner_name, U.login as onwer_login, U.uid as owner_id FROM multimedia M INNER JOIN shareables S ON S.id = M.id INNER JOIN users U ON U.uid = S.owner_id WHERE M.id = ?', [sid], function(err, data){
			if(err){
				callback(null);
				return;
			} else {
