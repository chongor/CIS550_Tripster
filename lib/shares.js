...
var newslib = require("newsfeed.js");
			...
			if(!success){
				...
				db.query('INSERT INTO shareable_albums (id, updateTime, title, description) VALUES (?, NOW(), ?);',
					...
					if(err){
						...
						db.query('SELECT * FROM shareable_albums WHERE id = ?', sid,function(err, data){
							...
							if(err){
								...
								var s_album = {
									'id': data[0].id,
									'updateTime': data[0].udpateTime;
									'title': data[0].title,
									'description': data[0].description
								}

								newslib.post(uid, uid, s_album, 0, function(){
									callback(true, sid);
									return;
								});
							}
