...
module.exports = function(db){
	...
	this.getAlbumCover = function(a_id, callback){
		// Albums may only contain photos or videos
		db.query('SELECT AI.album_id AS album_id, AI.item_id AS id, S.type AS type, M.url AS url, M.title AS title, S.time AS time, S.owner_id AS owner_id, U.name AS owner_name, U.login AS owner_login, U.email AS owner_email FROM album_items AI INNER JOIN multimedia M ON M.id = AI.item_id INNER JOIN shareables S ON S.id = M.id INNER JOIN users U ON U.uid = S.owner_id WHERE AI.album_id = ? AND S.type IN ("photo") ORDER BY time DESC LIMIT 1', [a_id], function(err,data){
			if(err){
				console.log(err);
				callback(null);
				return;
	...
	this.getRating = function(s_id, start, end, callback){
		...
		db.query('SELECT SR.rating AS rating, SR.comment AS comment, SR.time AS time, U.uid AS rater_id, U.name AS rater_name, U.login AS rater_login, U.email AS rater_email FROM shareable_ratings SR INNER JOIN users U ON U.uid = SR.rater_id WHERE SR.item_id = ? ORDER BY SR.time DESC ' + limit + '; SELECT AVG(rating) AS avg_rating FROM shareable_ratings WHERE `item_id` = ?;', [s_id, s_id], function(err, data){
			if(err){
				console.log(err);
				callback(null, -1);
				return;
	...
	this.addRating = function( item_id, rating, comment, rater_id, callback){
		db.query('INSERT INTO shareable_ratings (item_id, rating, comment, rater_id) VALUES (?)', [[item_id, rating, comment, rater_id]], function(err, data){
			if(err){
	...
	this.isViewable = function(s_id, uid, callback){
		this.getShareable(s_id, function(record){
			...
			db.query('SELECT * FROM trip_members WHERE trip_id IN '
				+ '(SELECT trip_id FROM trip_shareables WHERE item_id = ?) AND role IN ("member:invite","member:request","admin","member") AND uid = ?', 
				[s_id, uid], function(err, data){
				if(err){
					callback(false, null);
					return;
				}else{
	...
	this.parentShareable = function(item_id, trip_id, privacy, callback){
		db.query('INSERT INTO trip_shareables (item_id, trip_id, privacy, time) VALUES(?, NOW())', [[item_id, trip_id, privacy ? privacy : 0]], function(err, data){
			if(err){
				callback(false, err);
				return;
			}
	...
	var parentToAlbum = function(item_id, album_id, callback){
		db.query('INSERT INTO album_items (album_id, item_id) VALUES (?)',
			[[album_id, item_id]], function(err, data){
			if(err){
				callback(false, err);
				return;
			} else {
	...
	this.createMediaShare = function(type, uid, title, url, parent, callback){
		if(['photo', 'video', 'url'].indexOf(type) < 0){
			type = 'url';
		}
		genShareable(type, uid, function(success, sid){
			if(!success){
				callback(false, sid);
				return;
			}
			db.query('INSERT INTO multimedia (id, title, url) VALUES (?)',[[sid, title, url]], function(err, data){
				if(err) {
					...
					parentToAlbum(sid, parent, function(success){
						if(success){
							callback(true, sid);
						}else{
							callback(false, sid);
