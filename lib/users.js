...
module.exports = function(db){
	...
	this.getRelationship = function(uid1, uid2, callback){
		db.query('SELECT * FROM friends WHERE (`src_uid` = ? AND `dest_uid` = ?) '
			+ 'OR (`src_uid` = ? AND `dest_uid` = ?)', 
			[uid1, uid2, uid2, uid1], function(err, data){
			if(err){
				callback("none");
				return;
			}else{
	...
	this.getFriends = function(uid, callback){
		db.query('SELECT * FROM users U INNER JOIN vFriends F ON F.friend = U.uid WHERE F.uid = ?',[uid], function(err,data){
			if(err){
				callback(null);
				return;
			}
			var friends = [];
			for(var i = 0; i < data.length; i++){
				friends.push(dbRecordToUser(data[i]));
			}
			callback(friends);
			return;
		});
	...
	this.updateUser = function(uid, fieldsToUpdate, callback){
		var self = this;
		this.getUser(uid, function(user){
			if(user === null){
				callback(false, 'User does not exist');
				return;
			}
			if(typeof fieldsToUpdate !== "object" || fieldsToUpdate === null){
				callback(false, 'No fields to update');
				console.log(fieldsToUpdate);
				return;
			};
			// User exists
			db.query("UPDATE users SET ? WHERE `uid` = ?", 
				[fieldsToUpdate, uid], function(err, data){
				if(err){
					callback(false, err);
					return;
				}else{
	...
	this.friendRequest = function(from, to, callback){
		this.getRelationship(from, to, function(rel){
			if(rel !== "to"){
				db.query("INSERT INTO friends (`src_uid`, `dest_uid`) VALUES(?, ?)", 
					[from, to], function(err, data){
					if(err){
						callback(false, err);
						return;
					}
	...
	this.unfriend = function(from, to, callback){
		this.getRelationship(from, to, function(rel){
			if(rel !== "none"){
				db.query("DELETE FROM friends WHERE (`src_uid` = ? AND `dest_uid` = ?) " + 
				"OR (`src_uid` = ? AND `dest_uid` = ?)", [from, to, to, from], function(err, data){
					if(err){
						callback(false, err);
						return;
					}
