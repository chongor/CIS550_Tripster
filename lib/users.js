		...
		if(login === null || password === null || fields === null ||
			...
			this.getUserByLogin(login, function(resp){
				if(resp !== null){
					...
					db.query('INSERT INTO users (name, login, password, privacy, address, phone, affiliation, interests) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
						[uobj.name, uobj.login, uobj.password, uobj.privacy, uobj.address, uobj.phone, uobj.affiliation, uobj.interests], function(err, data){
						if(err){
