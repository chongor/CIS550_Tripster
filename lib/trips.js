...
module.exports = function(db){
	...
	this.getTripByMembership = function(uid, callback) {
		...
		db.query('SELECT DISTINCT * FROM trips T INNER JOIN trip_members M ON M.trip_id = T.trip_id WHERE M.uid = ? AND M.role IN ("member","member:request", "member:invite") AND M.role <> "admin" AND T.owner_uid <> ?',
	...
	// Only being called when approving a request, not for invites
	...
	this.getRating = function(trip_id, start, end, callback){
		...
		db.query('SELECT TR.rating AS rating, TR.comment AS comment, TR.time AS time, U.uid AS rater_id, U.name AS rater_name, U.login AS rater_login, U.email AS email FROM trip_ratings TR INNER JOIN users U ON U.uid = TR.uid WHERE TR.trip_id = ? ORDER BY TR.time DESC ' + limit + '; SELECT AVG(rating) AS avg_rating FROM trip_ratings WHERE `trip_id` = ?;', [trip_id, trip_id], function(err, data){
			if(err){
				console.log(err);
				callback(null, -1);
				return;
	...
	this.getAlbums = function(trip_id, callback){
		db.query('SELECT TS.item_id AS item_id, TS.trip_id AS trip_id, TS.privacy AS privacy, TS.time AS time, U.uid AS uid, U.name AS name, U.login AS login, U.password AS password, U.address AS address, U.phone AS phone, U.email AS email FROM trip_shareables TS INNER JOIN shareables S ON S.id = TS.item_id INNER JOIN users U ON U.uid = S.owner_id WHERE TS.trip_id = ?', trip_id,function(err,data){
