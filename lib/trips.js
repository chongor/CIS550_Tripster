 ...
 */
 ...
 module.exports = function(db){
	...
	// Leonard: First argument of all callbacks is true if there is error, or false otherwise
	...
	this.isOwnerOfTrip = function(uid, tid, callback) {
		this.getTripByOwner(uid, function(tids) {
			if (tids !== null) {
				callback(false, _.contains(tids, tid));
			}
			else {
				callback(true);
			}
	...
	this.getOwnerOfTrip = function(tid, callback) {
		db.query('SELECT owner_uid FROM trips WHERE trip_id = ?', tid, function(err, owner) {
			if (err) callback(true);
				...
				callback(false, owner[0].owner_uid);
			}
		}.bind(this));
	};

	// Pass in uid if testing for isMemberOfTrip
	...
	this.inviteUserToTrip = function(inviter_uid, member_uid, tid, callback) {
		...
		this.isOwnerOfTrip(inviter_uid, tid, function(err, isOwner) {
			if (err || !isOwner) callback(true);
			else {
				// Add to trip_members
				db.query('INSERT INTO trip_members (trip_id, uid, role) VALUES (?)', [[tid, member_uid, 'invite']], function(inviteErr) {
						if (inviteErr) callback(true);
						else {
							// Send notification
							// First find how many notifications member_uid has
							db.query('SELECT COUNT(*) AS notificationCount FROM notifications WHERE owner_id = ?',
								member_uid, function(countErr, count) {
									if (countErr) callback(true);
									else {
										var nid = count[0].notificationCount + 1;
										// Get inviter's name
										db.query('SELECT name FROM users WHERE uid = ?', inviter_uid, function(nameErr, name) {
												if (nameErr) callback(true);
												else {
													var inviterName = name[0].name;
													db.query('INSERT INTO notifications (owner_id, nid, notification) VALUES (?)',
														[[member_uid, nid, inviterName + ' invited you to a trip!']], function(notificationsErr) {
															if (notificationsErr) callback(true);
															else callback(true, tid);
													}.bind(this));
												}
										}.bind(this));
								}
	...
	this.requestToJoinTrip = function(requester_uid, tid, callback) {
		...
		this.getOwnerOfTrip(tid, function(err, ownerId) {
			if (err) callback(true);
				...
				// Add to trip_members
				db.query('INSERT INTO trip_members (trip_id, uid, role) VALUES (?)', [[tid, requester_uid, 'request']], function(inviteErr) {
						if (inviteErr) callback(true);
						else {
							// Send notification
							// First find how many notifications owner has
							db.query('SELECT COUNT(*) AS notificationCount FROM notifications WHERE owner_id = ?',
								ownerId, function(countErr, count) {
									if (countErr) callback(true);
									else {
										var nid = count[0].notificationCount + 1;
										...
										db.query('SELECT name FROM users WHERE uid = ?', requester_uid, function(nameErr, name) {
												if (nameErr) callback(true);
													...
													db.query('INSERT INTO notifications (owner_id, nid, notification) VALUES (?)',
														[[ownerId, nid, requesterName + ' requested to join your trip!']], function(notificationsErr) {
															if (notificationsErr) callback(true);
															else callback(true, tid);
													}.bind(this));
												}
	...
	this.joinTrip = function(uid, tid, callback) {
		// Delete either 'request' or 'invite'
		db.query('DELETE FROM trip_members WHERE uid = ?', uid, function(deleteErr) {
			if (deleteErr) callback(true);
			else {
				db.query('INSERT INTO trip_members (trip_id, uid, role) VALUES (?)', [[tid, uid, 'member']], function(insertErr) {
					if (insertErr) callback(true);
					else {
						callback(false, tid);
					}
				}.bind(this));
	...
	// Notifications will be periodically removed elsewhere
